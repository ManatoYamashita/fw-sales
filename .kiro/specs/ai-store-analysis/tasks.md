...
- **Gemini API の `responseJsonSchema` サブセット制約**(2025-05-09 実機検証で発見): Gemini API は JSON Schema の限定的 subset のみサポート(`node_modules/@google/genai/dist/genai.d.ts:4551-4565` 参照)。Zod 4 の `z.toJSONSchema()` が出力する `$schema` メタフィールドや `maxLength` (`call_script.max(1500)` 由来) は **非対応**で 400 を返す。`lib/ai/schema.ts:stripUnsupportedKeys` で `$schema` / `minLength` / `maxLength` / `pattern` / `multipleOf` / `exclusive*` / `default` を再帰除去してから API に渡す。クライアント側 `validateAiAnalysis` で再検証する設計のため Gemini 側 schema は補助扱いで安全。
- **Gemini API の構造化出力と `tools` の同時設定不可**(2025-05-09 実機検証で発見): `responseMimeType: "application/json"` + `responseJsonSchema` と `tools: [{ urlContext: {} }]` を同時設定すると 400 (INVALID_ARGUMENT) で `Tool use with a response mime type: 'application/json' is unsupported`。SDK の TS 型は `unknown` で受けるため型チェック検出不可、実機リクエストで初発覚。本 spec は構造化出力を主軸とするため URL Context は **不使用**(削除済)、HTML 全文を `prompt.ts:buildAnalysisPrompt` で user Part として投入する単一経路で運用。`research.md` Topic 2 の初出記載「`responseJsonSchema` と併用可能」は誤情報のため訂正済。今後 URL Context が必要になる場合は本 spec とは別の経路(構造化出力を諦めるか、別 LLM 呼出に分割する)を設計すること。
