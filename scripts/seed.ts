 ...
 *
 * 役割:
 * - `lib/db/seed-data.ts` の `SEED_STORES` / `SEED_DEALS` / `SEED_HANDOFFS` の
 *   3 entity を Postgres (Supabase) にベキ等な upsert で投入する
 *   (Issue #110 で旧 `research` テーブルを撤去したため 4 → 3 entity)。
 *
 * 実行例:
 * - `pnpm seed`
 *
 ...
 * - Handoff は全フィールド primitive のため `toDbRow` 不要
...
import { stores, deals, handoffs } from "@/lib/db/schema";
...
async function main(): Promise<void> {
  ...
  // 親→子の順 (stores → deals → handoffs) で FK 整合を保つ
  ...
  console.log(
    `Seeded ${SEED_STORES.length} stores, ${SEED_DEALS.length} deals, ${SEED_HANDOFFS.length} handoffs.`,
  );
