...
export function SalesActivityForm({ store, deal, profiles, onClose }: { store: Store; deal?: Deal; profiles: readonly Profile[]; onClose: () => void }) {
  ...
  return <form action={submit} className="rounded-xl border border-border bg-card p-4 space-y-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <FormField label="実施日" htmlFor={`date-${deal?.id ?? "new"}`} required><Input id={`date-${deal?.id ?? "new"}`} name="date" type="date" required defaultValue={deal?.date ?? todayInTimeZone("Asia/Tokyo")} /></FormField>
      <FormField label="活動種別" htmlFor={`type-${deal?.id ?? "new"}`} required><Select width="full" id={`type-${deal?.id ?? "new"}`} name="meeting_type" defaultValue={deal?.meeting_type ?? "対面"}>{MEETING_TYPES.map((v) => <option key={v}>{v}</option>)}</Select></FormField>
      <FormField label="営業状態" htmlFor={`status-${deal?.id ?? "new"}`} required><Select width="full" id={`status-${deal?.id ?? "new"}`} name="status" value={status} onChange={(e) => setStatus(e.target.value as typeof status)}>{DEAL_STATUSES.map((v) => <option key={v}>{v}</option>)}</Select></FormField>
      <FormField label="営業担当者" htmlFor={`sales-${deal?.id ?? "new"}`}><Select width="full" id={`sales-${deal?.id ?? "new"}`} name="assigned_sales_user_id" defaultValue={deal?.assigned_sales_user_id ?? store.assigned_sales_user_id ?? ""}><option value="">未割当</option>{profiles.map((p) => <option key={p.id} value={p.id}>{p.display_name}</option>)}</Select></FormField>
