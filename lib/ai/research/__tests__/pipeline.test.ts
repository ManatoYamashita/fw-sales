...
describe("runStage1", () => {
  it("Source Discoveryを実行しSource Registryを構築する", async () => {
    mockRunSourceDiscovery.mockResolvedValue({
      text: `[SOURCE]\nurl: https://vertexaisearch.cloud.google.com/grounding-api-redirect/x\ntitle: 公式\ntype: official_site\nwhy_useful: y\n[/SOURCE]`,
      groundingMetadata: {
        groundingChunks: [
          { web: { uri: "https://vertexaisearch.cloud.google.com/grounding-api-redirect/x", title: "gorp.jp" } },
        ],
      },
    ...
    expect(result.sourceRegistry[0]!.source_type).toBe("official_site"); // enrichment適用
    expect(result.usageMetadata?.totalTokenCount).toBe(100);
  });

  it("groundingChunksが空でも空のSource Registryを返す(例外を投げない)", async () => {
    mockRunSourceDiscovery.mockResolvedValue({
      text: "何も見つかりませんでした",
...
describe("runStage2", () => {
  ...
  it("正常なJSON応答をパース・検証してitemsを返す", async () => {
    ...
    const result = await runStage2(
      { store: STORE, track: "FACT", sourceRegistry: REGISTRY },
  ...
  it("sourceRegistryが空でもAPI呼出自体は行う(Source Registry空時の案内はprompts.tsが担当)", async () => {
    ...
    const result = await runStage2(
      { store: STORE, track: "FACT", sourceRegistry: [] },
      AbortSignal.timeout(1000),
    );
...
describe("finalizeResearchItems", () => {
  it("FACT/ANALYSIS/HEARING項目を統合しdeterministic validationを適用する", () => {
    const factItems = [
      {
        key: "business_hours_holidays",
    ...
    // FACT項目(検証済みsourceあり) + 11件のHEARING系
