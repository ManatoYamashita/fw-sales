...
1. Google Searchで対象店舗を正確に同定してください。店舗名・住所・電話番号での基本同定に加え、
   ...
   **必ず実行する検索(mandatory)**: 上記の任意クエリとは別に、以下だけは店舗の状況に
   関わらず**毎回必ず1回は検索を試みてください**(結果が0件でも構いません。試みたことが
   [QUERY]から分かるようにしてください):
   - 「店舗名 + 食べログ」または \`site:tabelog.com 店舗名 エリア\`

   理由: グルメポータルの店舗ページには「店舗直通番号」と「予約・問い合わせ用の番号
   (転送番号)」が別々に併記されていることが多く、予約用番号は公式サイトや記事からは
   得られないことが大半だからです。営業上どちらの番号も有用です。

   **mandatoryなのは「検索を試みること」であって、「その結果を必ず採用すること」ではありません。**
   検索結果に該当ページのURLが実際に表示されなかった場合は、SOURCEを出力せずそのまま
   通常どおり調査を続行してください(「それらしいURL」を推測・組み立てて埋めることは
   絶対に禁止です。下記「URLに関する重要な注意」を厳守すること)。
   検索結果に対象店舗と一致するページのURLが実際に表示された場合は、
   SOURCE枠に余裕がある限りグルメ/予約サイト系の候補として優先的に残してください。
   なお、そこに掲載されているという事実そのものは、その内容が対象店舗のものであることや
   正しいことの保証にはなりません(店舗同定と内容確認は後続の工程で別途行われます)。
2. 有用な情報源を優先順位付きで探索してください: 公式サイト → 公式SNS → Googleマップ関連 → グルメサイト(食べログ・ホットペッパー・ぐるなび・Retty等) → 予約サイト → 地域公式・商店街 → 信頼できる記事・ブログ → 競合店舗情報 → 公的統計データ。
   ...
   **最低限、以下のカテゴリすべてについて一度は検索を試みてください**(すべてに情報源が
   ...
   地域メディア・記事 / 競合店舗 / 電話番号・予約導線。
   **「電話番号・予約導線」について**: 店舗の連絡先は、公式サイトのほかグルメ・予約
   ポータルの店舗ページに掲載されていることが多く、そこでは「店舗直通番号」と
   「予約・問い合わせ用の番号(転送番号)」が**別々に併記**されている場合があります。
   どちらも営業上有用なので、電話番号や予約導線を掲載している店舗ページを1件以上
   候補に入れられるよう探索してください(該当ページが存在しない店舗もあります。
   その場合は無理に埋めず、他の情報源で通常どおり続行してください)。
...
# URLに関する重要な注意(絶対に守ること)
- SOURCEのurlは、**検索結果に実際に表示されたURLをそのまま書き写す**ことのみ許可します。
- 特に食べログ・ホットペッパー・Retty等、店舗ごとに固有のID(数字・英数字の店舗コード)を
  URLパスに含むサイトについて、**そのIDを推測・生成・組み立てることは絶対に禁止**します。
  例えば「炉端ジュンという店名を知っているから、ホットペッパーのURLは
  https://www.hotpepper.jp/strJ0038xxxxx/ のはずだ」のように、正しい店舗名を知っている
  ことを理由に別のIDのURLを作り出してはいけません。それは実在しない、または全く別店舗の
  ページになりえます。
- titleとurlは、必ず**同一の検索結果**から取得したペアにしてください。あるURLに、別の
  検索結果で見た店名をtitleとして付け替えることは禁止します。
- 検索結果に実際のURLが表示されなかった場合は、そのサイトについてSOURCEを出力しないで
  ください(「それらしいURL」を組み立てて埋めるより、そのSOURCEを省略するほうが安全です)。
...
[SOURCE]
url: (検索結果に実際に表示されたURLをそのまま。推測・組み立て禁止)
title: (同じ検索結果に表示されたタイトル)
...
代表転送番号)は、単純に"conflict"にしないでください。用途自体が不明で本当に内容が
矛盾する場合のみ"conflict"としてください。

**用途が異なる番号は捨てずに、役割ラベル付きで全て併記してください。**
グルメサイトの予約用050番号は営業上有用なため、店舗直通番号と併せて残します。
1つの文字列に " / " 区切りで、店舗直通番号を先頭に書いてください。例:

- 1件のみの場合: 045-305-6536
- 複数ある場合: 店舗直通: 045-305-6536 / 予約・問い合わせ(食べログ): 050-5869-4190

役割ラベルは本文で確認できた表記に基づいて書いてください(「予約・お問い合わせ」等)。
**050番号を「店舗直通」と書いてはいけません。** 用途が本文から判断できない番号は
役割ラベルを付けずに併記するか、確信が持てなければ含めないでください。

**valueに書いた電話番号は、必ずevidenceにも同じ番号を書いてください。**
evidenceに現れない番号はコード側の検証で自動的に除外されます(存在しない番号を
生成しないこと)。

**status="conflict"とする場合も同様です。各candidateのvalueに書いた電話番号は、
そのcandidateのevidenceにも同じ番号を必ず記載してください。** 記載が無いcandidateは
コード側の検証で候補から除外されます。また、conflictの各candidateには、その候補を
確認できたsource_idsを正確に対応付けてください(実際に本文で確認していない情報源を
candidateのsource_idsに含めないこと)。`;
...
# 店舗同定(重要、Source Registryの各URLごとに厳密に行うこと)
**「URLの取得に成功したこと」と「そのページが対象店舗について書かれていること」は別のことです。**
検索結果のtitleがそれらしい店名になっていても、実際に取得した本文が全く別の店舗・別の場所に
ついてのページであることがあります(例: 店名の文字列だけが似ている、URLのIDだけが誤って別
店舗を指している等)。**タイトルやURLの見た目を信用せず、必ず取得した本文の実際の内容で
判定してください。**
...
- relation: 取得した本文の内容から判断した、対象店舗との関係。
  - "target_store": 本文が対象店舗そのものについて書かれている。
  - "competitor": 本文が対象店舗ではなく、競合店舗(調査目的で探した別店舗)について
    書かれている。
  - "contextual": 対象店舗個別のページではないが、エリア・商圏等の文脈情報として有用。
  - "unrelated": 対象店舗にも競合調査にも無関係(誤ったURL、別業種、別地域等)。
  - "uncertain": 本文からは判断できない、または本文を十分に確認できなかった。
- observed_title / observed_name / observed_address / observed_phone: **そのURL本文で
...
別店舗のページなのに"target_store"と申告することのないよう、本文の内容だけを根拠に
慎重に判定してください。

さらに、run全体としての総合的な同定結果を store_identification フィールド
(matched_name/matched_address/identification_note)にも記載してください
(これは個別sourceの判定を代替するものではなく、run全体の参考情報です)。
...
**evidence本文に"S01"等のsource ID表記を書き込まないでください。** 出典の対応関係は
source_idsフィールドのみで表現し、evidence文章はsource IDに依存しない自然な説明文に
してください(例: 「グルメサイトによると」であって「S05によると」ではない)。
